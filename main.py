...
color = 0x00a1d8
administrators = 487721974773317633
...
async def stock(ctx):
 if ctx.channel.type != discord.ChannelType.private:
        ...
        filefile.close()
        embed=discord.Embed(title="Follows Stock",color=3447003, description=f"Twitch Stock:\n \nFollow: **{fnum_lines}**\nSpam: **{snum_lines}** ")
...
async def Status_Rotate(seconds=5):
    ...
    activity = discord.Activity(type=discord.ActivityType.watching, name=f'discord.gg/zedsrewardz')
    await bot.change_presence(activity=activity)
    await asyncio.sleep(30)
...
async def bronze(ctx):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    embed=discord.Embed(title="Receive Bronze",color=3447003, description=f"**Set discord.gg/zedsrewardz in your status, you will automatically get a rank Bronze**")
    await ctx.send(embed=embed)
...
async def on_member_update(before, after):
    role_id = 1007617959646203955
    role = get(before.guild.roles, id=role_id)
    if 'discord.gg/zedsrewardz' in str(before.activities):
      if 'discord.gg/zedsrewardz' in str(after.activities):
        pass
      else:
        await after.remove_roles(role)
        channel = bot.get_channel(1007623451110031420)
...
async def help(ctx):
    ...
    if ctx.channel.type != discord.ChannelType.private:
            embed = discord.Embed(title='__Zeds Followers__',description='Bot Commands.', color=color)
            embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/990791102221131877/1004956523568582737/Screenshot_2022-08-04_232951.png')
            ...
            embed.add_field(name='Bronze', value=f'```{prefix}bronze```', inline=False)
            await ctx.send(embed=embed)


@bot.command()
async def ceohelp(ctx):
    config_file = get_config()
    json_object = json.loads(config_file)
    prefix = json_object['bot_config']["prefix"]
    print(f'{ctx.author} | {ctx.author.id} -> {prefix}help')
    embed = discord.Embed(title='__Zeds__',description='Bot Commands.', color=color)
    embed.set_thumbnail(url='https://media.discordapp.net/attachments/941230578144006174/979290800171155466/f08d8d8fcc0b8dcc8dec718a5719f142.png')
...
async def clear(ctx):
    ...
    if ctx.channel.type != discord.ChannelType.private:
        if ctx.author.guild_permissions.administrator:
            ...
            await ctx.send('**Cleared by Snowyz!**')
        else:
            await ctx.message.delete()

@bot.command()
async def tfollow(ctx, arg):
 logschannel = bot.get_channel(1007623451110031420)   
 if ctx.channel.type != discord.ChannelType.private:
    config_file = get_config()
    json_object = json.loads(config_file)
